...
cloud_sql_instance = pulumi_gcp.sql.DatabaseInstance(
    ...
    settings=pulumi_gcp.sql.DatabaseInstanceSettingsArgs(tier="db-f1-micro"), # TODO: from config
)
...
users = pulumi_gcp.sql.User(
    ...
    password=config.require_secret("db-password"), # TODO: Pulumi.random if not specified
)
